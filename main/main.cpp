...
<<<<<<< HEAD printf("Booting... waiting for stabilization\n"); // message de debug vTaskDelay(pdMS_TO_TICKS(1000)); // attente de 1 sec pour ne plus avoir valeur
...
>>>>>>> 6529d3f9b116523d76b1d92b84388d0b6768ebbc
  ...
  constexpr int kUpdateDelayMs =
      50; // delay time in ms for the led panel update
  ...
  void app_main(void) {
    ...
    while (true) {
      myLedPanel.updateMatrix();
      vTaskDelay(
          pdMS_TO_TICKS(100)); // provisoire. Devra être géré par la task.
      myLedPanel.shiftLedColor myLedPanel.litLedPanel();
    };
